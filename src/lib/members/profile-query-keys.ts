...
export type ProfileRangeChoice = "all" | "semester" | "academic" | "year" | "custom";
...
export function parseProfileLocation(search: URLSearchParams): {
  ...
  const valid = validDate(from) && validDate(to) && (!from || !to || from <= to);
  return {
    valid,
...
export const profilePath = (location: ProfileLocation) => path("/api/profile", location);
export const profilePagePath = (location: ProfileLocation) => path("/profile", location);
...
export function profilePresets(today: string) {
  ...
  const academic = month >= 7
    ? { from: `${year}-07-01`, to: `${year + 1}-06-30` }
    : { from: `${year - 1}-07-01`, to: `${year}-06-30` };
  const date = new Date(`${today}T12:00:00Z`);
  date.setUTCFullYear(date.getUTCFullYear() - 1);
  date.setUTCDate(date.getUTCDate() + 1);
  return { semester, academic, year: { from: date.toISOString().slice(0, 10), to: today } };
}
...
export function selectedProfileRange(
  ...
  return (Object.entries(presets).find(([, range]) =>
    range.from === location.from && range.to === location.to,
  )?.[0] as Exclude<ProfileRangeChoice, "all" | "custom"> | undefined) ?? "custom";
}
